...
def kmeans_selection(in_seqs, unlabeled, embeddor, select_bs=100, device=device): 
    # check device 
    if in_seqs.device != device: 
        in_seqs = in_seqs.to(device)
    if embeddor.device != device: 
        embeddor = embeddor.to(device)
    # extract embedding ensemble  
    grads = embeddor(input_ids=in_seqs[unlabeled])
...
def main(args): 
    ...
    while current_selection_round < selection_rounds:
        ...
        # ================= KMEANS Selection ================= 
        selection_indices = kmeans_selection(trainin, unlabeled, model, selection_batch_size)
        ...
        trainer = Trainer(
            ...
            callbacks=[
                ...
                TestLossCallback(logging_dir=training_args.logging_dir, file_name=f"kmeans_bs={selection_batch_size}_round={current_selection_round}_v{version}.csv")
            ], 
        )
