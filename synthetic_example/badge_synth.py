...
class BertEmbeddor(BertModel):
    ...
    def extract_gradient(self, input_ids, labels=None): 
        if labels is None: 
            print("Must provide label to extract gradient.")
            return None 
        super().zero_grad()
        output = super().forward(input_ids=input_ids, output_hidden_states=True)
        hidden_out = output.last_hidden_state[...,:self.out_size]    
        gradient = []
        def save_grad(grad): 
            gradient.append(grad)
        hidden_out.register_hook(save_grad)
        loss = self.loss(hidden_out, labels)
        loss.backward()
        return gradient[0].detach() if gradient else None
...
def badge_selection(in_seqs, out_seqs, unlabeled, embeddor, select_bs=100, device=device): 
    # check device 
    if in_seqs.device != device: 
        in_seqs = in_seqs.to(device)
    if out_seqs.device != device: 
        out_seqs = out_seqs.to(device)
    if embeddor.device != device: 
        embeddor = embeddor.to(device)
    # extract gradient ensemble  
    grads = embeddor.extract_gradient(input_ids=in_seqs[unlabeled], labels=out_seqs[unlabeled])
    grads = grads.flatten(start_dim=1).detach().cpu()
    # run kmeans to select indices
    print("Clustering the gradient...")
    kmeans = KMeans(n_clusters=select_bs, random_state=10, init='k-means++', n_init="auto").fit(grads)
    centroids = kmeans.cluster_centers_
    closest, _ = pairwise_distances_argmin_min(centroids, grads)
    selected_data_indices = [unlabeled[i] for i in closest]
...
def main(args): 
    ...
    while current_selection_round < selection_rounds:
        ...
        # ================= BADGE Selection ================= 
        selection_indices = badge_selection(trainin, trainout, unlabeled, model, selection_batch_size)
        ...
        trainer = Trainer(
            ...
            callbacks=[
                ...
                TestLossCallback(logging_dir=training_args.logging_dir, file_name=f"badge_bs={selection_batch_size}_round={current_selection_round}_v{version}.csv")
            ], 
        )
