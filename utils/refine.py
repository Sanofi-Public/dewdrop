...
def refine(input_file, output_file, check_for_strained_bonds=True, tries=3, n=6, n_threads=-1, platform=None, CudaDeviceIndex=0):

    for i in range(tries):
        if refine_once(input_file, output_file, check_for_strained_bonds=check_for_strained_bonds, n=n, n_threads=n_threads, platform=platform, CudaDeviceIndex=CudaDeviceIndex):
            return True
    return False


def refine_once(input_file, output_file, check_for_strained_bonds=True, n=6, n_threads=-1, platform=None, CudaDeviceIndex=0):
    ...
    for i in range(n):
        try:
            simulation = minimize_energy(topology, positions, k1=k1, k2 = k2, n_threads=n_threads, platform=platform, CudaDeviceIndex=CudaDeviceIndex)
            ...
            if check_for_strained_bonds:
                ...
                try:
                    ...
                    if len(strained_bonds) > 0:
                        needs_recheck = True
                        topology, positions = strained_sidechain_bonds_fixer(strained_bonds, topology, positions, n_threads=n_threads, CudaDeviceIndex=CudaDeviceIndex)
                    else:
                        needs_recheck = False
                except OpenMMException as e:
                    topology, positions = fixer.topology, fixer.positions
...
def minimize_energy(topology, positions, k1=2.5, k2=2.5, n_threads=-1, platform=None, CudaDeviceIndex=0):
    ...
    if platform=="CUDA":
        ...
        simulation = app.Simulation(modeller.topology, system, integrator, platform, {'CudaDeviceIndex': str(CudaDeviceIndex)})
    else:
        if n_threads > 0:
    ...
    # print(simulation.context.getPlatform().getName(), flush=True) # Incase if one needs to know the platform being used
    # Minimize the energy
...
def strained_sidechain_bonds_fixer(strained_residues, topology, positions, n_threads=-1, CudaDeviceIndex=0):
    ...
    tmp_str=''.join(random.choice(string.ascii_letters) for i in range(20))
    tmp_file = f"side_chain_fix_tmp_{tmp_str}.pdb"
