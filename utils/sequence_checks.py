...
def number_single_sequence(sequence, chain, scheme="imgt", allowed_species=['human','mouse']):
    ...
    # DEBUG: "Seq provided as an H chain is not a H chain." 
...
def number_sequences(seqs, scheme="imgt", allowed_species=['human','mouse']):
    # ADDED: check for unrecognized chain
    numbered_seqs = {}
    for chain in seqs: 
        try: 
            numbered_seqs[chain] = number_single_sequence(seqs[chain], chain, scheme=scheme, allowed_species=allowed_species)
        except AssertionError as ae: 
            print(f"Caught invalid generated sequence: {ae}")
