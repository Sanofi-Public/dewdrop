...
from utils.cg import NUM_CG_TYPES
...
from utils.utils_data import MAX_DIST, NUM_EDGE_TYPE
...
class BesselBasis(torch.nn.Module):
    ...
    def __init__(self, rc, radial_num_basis=16) -> None:
        ...
        bessel_weights = (
            torch.linspace(start=1.0, end=self.radial_num_basis, steps=self.radial_num_basis)
            * math.pi
        )
        self.bessel_weights = torch.nn.Parameter(bessel_weights)

    def forward(self, x):
        return self.prefactor * torch.sin(
            self.bessel_weights[None, None, :] * x[:, :, None] / self.rc
        )  # / x.unsqueeze(-1)
...
class RadialNN(torch.nn.Module):
    def __init__(
        ...
        self.mlp = MLP(
            [
                self.radial_num_basis + self.num_edge_features
                if self.include_edge_features
                else self.radial_num_basis
            ]
...
class Emb(torch.nn.Module):
    def __init__(self, num_node_types, nc) -> None:
        super().__init__()
        ...
        self.embed_s = torch.nn.Embedding(
            num_node_types,
            self.d_s,
            padding_idx=-1,
            max_norm=1,
        ...
        self.embed_v = torch.nn.Embedding(
            num_node_types,
            self.d_v,
            padding_idx=-1,
            max_norm=1,
...
class NN(pl.LightningModule):
    def __init__(
        ...
        fape_clip_val=10.0,
        scale_only_fape_clip_val=True,
        weight_struct_loss=1.0,
        weight_struct_loss_scale="constant",
        attn_dropout=0.0,
        num_recycling=0,
        block_start_recycling=-1,
        compute_torsion_angle=False,
        weight_torsion_loss=1.0,
    ):
        super().__init__()
        ...
        self.lr_anneal_final_step_frac = lr_anneal_final_step_frac
        ...
        self.scale_only_fape_clip_val = scale_only_fape_clip_val
        ...
        self.block_start_recycling = block_start_recycling
        self.compute_torsion_angle = compute_torsion_angle
        self.weight_torsion_loss = weight_torsion_loss

        if self.distinct_embeddings:
            self.embs = torch.nn.ModuleList(
                [Emb(NUM_CG_TYPES + 1, self.nc) for _ in range(self.num_blocks)]
            )
        else:
        ...
        self.atom37_to_torsion_angles = atom37_to_torsion_angles()

        # prediciton results aggreated over an epoch 
    ...
    def freeze_layer(self, unfreeze_block=[]):
        """Freezes all blocks by setting required_grad = false EXCEPT blocks in unfreeze_block"""
    ...
    def recycling_block(
        ...
        current_iter_num = self.num_blocks

        for num_iter in range(1, nums_of_recycling + 1):
            for i in range(self.block_start_recycling, self.num_blocks + 1):
                if i == 0:
                    print("Start recycling at block 0, exiting...")
                    sys.exit()
                ...
                if return_struct or compute_loss:
                    ...
                    x_pred = compute_x_pdb(
                        X_v_pred, data["scatter_index"], data["scatter_w"], len(data["dst_resnum"])
                    )
                ...
                if compute_loss:
                    ...
                    weight_arg = None
                    if "weight_fwr_cdr_map_cg_resnum_uv" in data:
                        weight_arg = data["weight_fwr_cdr_map_cg_resnum_uv"]
                    loss = compute_FAPE_uv(
                        X_uv,
                        mask_atom_uv,
                        X_uv_pred,
                        eps=1e-4,
                        d_max=self.fape_clip_val,
                        weights=weight_arg,
                        scale_only_clamp_value=self.scale_only_fape_clip_val
                    )
                    #                 return_dict["losses_fape"][i].append(loss.detach())
                    return_dict["losses_fape"][current_iter_num].append(loss.detach())

                    # torsion angle loss
                    loss_torsion_angle = 0
                    if self.compute_torsion_angle:
                        all_atom_positions_, all_atom_mask_, aatype_ = to_atom37(
                            x_pred, data.dst_resnum, data.dst_atom, data.dst_resname
                        )
                        prot_obj = {
                            "aatype": aatype_,
                            "all_atom_positions": all_atom_positions_,
                            "all_atom_mask": all_atom_mask_,
                        }

                        prot_obj = self.atom37_to_torsion_angles(prot_obj)
                        loss_torsion_angle = torsion_angle_loss(
                            prot_obj["torsion_angles_sin_cos"],
                            data["torsion_angles_sin_cos"],
                            data["alt_torsion_angles_sin_cos"],
                            prot_obj["torsion_angles_mask"],
                        )
                        return_dict["losses_torsion_angle"][current_iter_num].append(
                            loss_torsion_angle.detach()
                        )
                    else:
                        return_dict["losses_torsion_angle"][current_iter_num].append(0)
                    ...
                    return_dict["losses_bond"][current_iter_num].append(loss_bond.detach())
                    return_dict["losses_angle"][current_iter_num].append(loss_angle.detach())
                    return_dict["losses_clash"][current_iter_num].append(loss_clash.detach())
                    loss_struct = loss_bond + loss_angle + loss_clash
                    if self.weight_struct_loss > 0.0:
                        if self.weight_struct_loss_scale == "constant":
                            scale = 1.0
                        elif self.weight_struct_loss_scale == "linear":
                            #                         scale = i / self.num_blocks
                            scale = current_iter_num / total_iter_num
                        elif self.weight_struct_loss_scale == "quadratic":
                            scale = (i / self.num_blocks) ** 2
                        #                             scale = (current_iter_num/ total_iter_num) **2
                        else:
                            raise ValueError
                        loss = (
                            loss
                            + tau * self.weight_struct_loss * loss_struct * scale
                            + self.weight_torsion_loss * loss_torsion_angle
                        )

                    if i > 0:
                        if is_train:
                            loss_for_grad = (
                                loss / self.accumulate_grad_batches / N
                            )  # normalization added # grad acc does appear here correctly
                            if not self.distinct_blocks:
                                assert not self.distinct_embeddings
                                #                             loss_for_grad = loss_for_grad / self.num_blocks
                                loss_for_grad = loss_for_grad / total_iter_num
                            ...
                            if (
                                i == self.num_blocks
                                and b == (N - 1)
                                and num_iter == nums_of_recycling
                            ):
    ...
    def forward(
        ...
        return_struct=True,
        is_train=False,
        set_RT_to_ground_truth=False,
        skip_first=False,
        save_grad=False,
    ):
        # recycling
        if is_train:
            # sample randomly from 0 to self.num-recycling
            nums_of_recycling = random.randint(0, self.num_recycling)
        else:
            nums_of_recycling = self.num_recycling
        total_iter_num = self.num_blocks + nums_of_recycling * (
            self.num_blocks - self.block_start_recycling + 1
        )

        return_dict = {
            "uid": [],
            "chain_id": [],
            # to recover original atom
            "dst_resnum": [], 
            "dst_resname": [],
            "dst_atom": [],
            "losses_fape": [
                [] for _ in range(total_iter_num + 1)
            ],  # eventually, list of floats; averaged over block; list comprehension necessary
            "losses_bond": [[] for _ in range(total_iter_num + 1)],
            "losses_angle": [[] for _ in range(total_iter_num + 1)],
            "losses_clash": [[] for _ in range(total_iter_num + 1)],
            "losses_torsion_angle": [[] for _ in range(total_iter_num + 1)],
        ...
        for b, data in enumerate(batch): 
            ...
            return_dict["uid"].append(data['uid'])
            return_dict["chain_id"].append(data['chain_id'])
            if "dst_resnum" in data: 
                return_dict["dst_resnum"].append(data['dst_resnum'])
            if "dst_resname" in data: 
                return_dict["dst_resname"].append(data['dst_resname'])
            if "dst_atom" in data: 
                return_dict["dst_atom"].append(data['dst_atom'])
            
            dtype = X0.dtype
            R_pred, T_pred = compute_init_struct(self.init_scheme, resnum, dtype)

            if compute_loss:
                # DEBUG: Training output is nan 
                # tqdm.write(f"What variables are nan? (batch = {b})\n")
                ...
                tau = min(1.0, self.trainer.global_step / self.warmup_steps) if is_train else 1.0

                # Commented for fine-tuning
            ...
            with torch.no_grad():
                edge_type_ij = (
                    torch.clamp(resnum[None, :] - resnum[:, None], min=-MAX_DIST, max=MAX_DIST)
                    + MAX_DIST
                )
                edge_type_ij[
                    edge_type_ij == (2 * MAX_DIST)
                ] = 0  # treat min / max dist as equal "far away"
            ...
            for i in range(self.num_blocks + 1):
                ...
                if i > 0:
                    ...
                    if i == self.num_blocks: 
                        # s is our fixed sized embedding
                        # v is variable size, because it's rotated with the rotational matrix (don't ask me why rotation changes size...)
                        return_dict['final_block_emb'] = s
                ...
                if compute_loss:
                    ...
                    if self.compute_torsion_angle:
                        ...
                        return_dict["losses_torsion_angle"][i].append(loss_torsion_angle.detach())
                    else:
                        return_dict["losses_torsion_angle"][i].append(0)
                    ...
                    if i > 0:
                        if is_train:
                            ...
                            # https://github.com/Lightning-AI/lightning/discussions/10792#discussioncomment-1712526
                            # only sync after last block of last sample?
                            if i == self.num_blocks and b == (N - 1) and nums_of_recycling == 0:
                                self.manual_backward(loss_for_grad)
                            else:
                                with self.trainer.model.no_sync():
                                    self.manual_backward(loss_for_grad)
                    ...
                    # extracting gradient from the final attention layer
                    if i == self.num_blocks and save_grad: 
                        return_dict['final_block_grad'] = []
                        # save grad
                        for param in list(self.enns[-1].layers[-1].parameters()): 
                            return_dict['final_block_grad'].append(param.grad.view(-1))
                        return_dict['final_block_grad'] = torch.cat(return_dict['final_block_grad'])

            # Start recycling
            if nums_of_recycling > 0:
                if compute_loss:
                    self.recycling_block(
                        R_pred,
                        T_pred,
                        edge_type_ij,
                        return_struct,
        ...
        # final averages
        if compute_loss:
            for c in ["fape", "angle", "bond", "clash", "torsion_angle"]:
                return_dict[f"losses_{c}"] = [
                    sum(x) / N for x in return_dict[f"losses_{c}"]
                ]  # grad acc does not appear here correctly
            #             return_dict["loss_total"] = return_dict["loss_total"] / N / self.num_blocks # mean over blocks
            return_dict["loss_total"] = (
                return_dict["loss_total"] / N / total_iter_num
    ...
    def configure_optimizers(self):
        ...
        self.lr_anneal_final_step = round(self.lr_anneal_final_step_frac * self.trainer.estimated_stepping_batches)
        self.warmup_steps = round(self.warmup_steps_frac * self.trainer.estimated_stepping_batches)
        
        print(f'The variable estimated_stepping_batches is: {self.trainer.estimated_stepping_batches}')
        print(f'The variable lr_anneal_final_step is: {self.lr_anneal_final_step}')
        print(f'The variable warmup_steps is: {self.warmup_steps}')
    ...
    def training_step(self, train_batch, batch_idx):
        opt = self.optimizers()
        return_dict = self(
            train_batch, compute_loss=True, return_struct=False, is_train=True, skip_first=True
        )
            ...
            if self.lr_warmup or self.lr_anneal:
                if self.trainer.global_step < self.warmup_steps:
                    ...
                    if step_after_warmup < self.lr_anneal_final_step:
                        arg = (math.pi / 2.0) * (step_after_warmup / self.lr_anneal_final_step)
        ...
        self.log(
            "train_loss", return_dict["loss_total"], batch_size=len(train_batch), sync_dist=True
        )
        for c in ["fape", "angle", "bond", "clash", "torsion_angle"]:
            self.log(
                f"train_loss_{c}_final",
                return_dict[f"losses_{c}"][-1],
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        self.log(
            "test_loss", return_dict["loss_total"], batch_size=len(test_batch), sync_dist=True
        )
        for c in ["fape", "angle", "bond", "clash", "torsion_angle"]:
            self.log(
                f"test_loss_{c}_final",
                return_dict[f"losses_{c}"][-1],
    ...
    def predict_step(self, pred_batch, batch_idx, dataloader_idx=0):
        # return the predicted coordinates of all atoms
        return_dict = self(pred_batch, compute_loss=False, return_struct=True, is_train=False, set_RT_to_ground_truth=False)
        # return torch.nn.utils.rnn.pad_sequence([b[-1] for b in return_dict['X_pred']], batch_first=True) # (#batch, resnum, CG, coord)
        return return_dict
...
class DTPByHead(torch.nn.Module):
    ...
    def forward(self, s, v, r_ij_vec, weights):
        ...
        w_ss, w_sv, w_vs, w_vv = rearrange(
            weights, "i j (c h m) -> c h i j m", c=4, h=self.num_heads
        )
...
class Equiformer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.pre_attn_dtp_linear = DTPByHead(
            nc_middle, nc_middle, nc_s_out_by_head, nc_v_out_by_head, num_heads
        )
        self.radialnn = RadialNN(
            self.pre_attn_dtp_linear.weight_numel,
            self.rc,
    ...
    def forward(self, s, v, edges_ij, r_ij, r_ij_vec, weight_cutoff=None):
        ...
        # post tp linear
        s_ij = (
            torch.einsum("h m n, h i j n -> h i j m", self.w_s_init, s_ij)
            + self.b_s_init[:, None, None, :]
        )
        ...
        # apply linear
        s_ij = (
            torch.einsum("h m n, h i j n -> h i j m", self.attn_msg_w_s, s)
            + self.attn_msg_b_s[:, None, None, :]
        )
        ...
        # -- Apply dropout to attn score
        a_ij = self.attn_dropout(a_ij)
...
class E3NN(torch.nn.Module):
    def __init__(
        ...
        self.include_edge_features = include_edge_features
        self.embed_edge = torch.nn.Embedding(
            num_edge_types,
            self.d_embed_edge,
            padding_idx=-1,
            max_norm=1,
        ...
        for _ in range(self.num_layers):
            self.layers.append(
                UpdateModule(self.irreps, self.irreps, apply_resnet=True)  # in  # out
            )
        # final layer predicts transformation
        self.layer_euclidean = UpdateModule(self.irreps, (0, 2), apply_resnet=False)  # out
    ...
    def forward(self, s, v, R, T, edge_type_ij):
        ...
        with torch.no_grad():
            ...
            weight_cutoff = (
                compute_weight_cutoff(r_ij, self.rc) if not self.disable_cutoff else None
            )
