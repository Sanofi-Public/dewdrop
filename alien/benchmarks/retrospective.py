...
from ..utils import shift_seed, create_directory, list_directory
from .oracle import SetOracle
from .metrics import (
    RMSE, MAE,
    F1, AUC, Accuracy, BalancedAccuracy, Precision, Recall,
    Scatter, TopScore
)
from ..models import Regressor, PytorchModel
...
def run_experiments(
    ...
    wandb_run_name=None,
    wandb_group=None,
    wandb_config={}, # automatically adds a bunch of things to this
    wandb_tags=[],
):
    """
    ...
    if stop_frac is not None:
        ...
        final_score = getattr(Scatter(
            np.asarray(test_samples.y.data), 
            np.array(model.predict(test_samples.X.data))), 
        score[0])()
        print("Done.")
    else:
        final_score = None
    ...
    if wandb:
        ...
        if 'name' not in wandb_args:
            if 'group' in wandb_args:
                ...
                wandb_args['name'] = wandb_args['group'] = f"{model.__class__.__name__} {selector.__class__.__name__} {secrets.token_bytes(4).hex()[:8]}"

        wandb_args['config'] = {
            'selector': selector.__class__.__name__, 'selector_args': selector_args, 
            'model': model.__class__.__name__, 'fit_args': fit_args,
            'n_runs': n_runs, 'scores': score, 'random_seed': random_seed,
            'n_initial': n_initial, 'batch_size': batch_size, 'num_samples': num_samples,
            'split_seed': split_seed, 'test_size': test_size, 'stop_samples': stop_samples,
            'stop_score': stop_score, 'stop_frac': stop_frac, 'final_score': final_score,
            'peek_score': peek_score,} | wandb_config
...
def _run_retro_iter(  # NOSONAR
    ...
    scores = [
        score_dict[name](name=name, file_path=os.path.join(run_dir, f'{name}.pickle'), scatter=scatter, use_wandb=bool(wandb_args))
...
def _check_retro_break(
    ...
    if stop_samples is not None and len(labelled_samples) >= stop_samples:
        ...
        return True
    if stop_frac is not None and (first_score - scores[0].y[-1]) >= stop_frac * (
