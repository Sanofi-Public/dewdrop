...
class Tumpy:
    ...
    def sum(a, axis=None, dtype=None, out=None, keepdims=False):
        if axis is None:
            return tr.sum(tr.as_tensor(a), dtype=dtype_n_to_t.get(dtype, dtype)).item()
        return TumpyTensor(tr.sum(tr.as_tensor(a), axis, dtype=dtype_n_to_t.get(dtype, dtype), out=out, keepdim=keepdims))
    ...
    def eye(N, M=None, k=0, dtype=float, device=None, order="C", like=None):
        if M is None:
            return TumpyTensor(tr.eye(N, dtype=dtype_n_to_t.get(dtype, dtype), device=device))
        return TumpyTensor(tr.eye(N, M, dtype=dtype_n_to_t.get(dtype, dtype), device=device))
    ...
    class linalg:
        ...
        def svd(a, full_matrices=True, compute_uv=True):
            if compute_uv:
                ...
                return TumpyTensor(tr.linalg.svdvals(tr.as_tensor(a)))
...
class TumpyTensor(tr.Tensor):
    ...
    def sum(self, axis=None, dtype=None, out=None, keepdims=False, initial=None, where=None):
        if axis is None:
            return tr.sum(tr.as_tensor(self), dtype=dtype_n_to_t.get(dtype, dtype)).item()
        return TumpyTensor(tr.sum(tr.as_tensor(self), axis, dtype=dtype_n_to_t.get(dtype, dtype), out=out, keepdim=keepdims))
