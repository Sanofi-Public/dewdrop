...
def entropy_from_covariance(cov, epsilon=0., rel_epsilon=None):
    """
...
def joint_entropy_from_covariance(  # NOSONAR
    ...
    if isinstance(cov, Iterator):
        ...
        for c_ij, i, j in zip(cov, *np.tril_indices(n_blocks, -1)):
            ...
            jen_block = entropy_from_covariance(cov_2x2, epsilon=0.0)
            jen[block_edges[i, 0] : block_edges[i, 1], block_edges[j, 0] : block_edges[j, 1]] = jen_block
            if i != j:
                jen[block_edges[j, 0] : block_edges[j, 1], block_edges[i, 0] : block_edges[i, 1]] = jen_block.T
        ...
        for block, (i_0, i_1) in zip(diag, block_edges):
            jen[i_0:i_1, i_0:i_1] = joint_entropy_from_covariance(block, epsilon=0)
            if pbar:
                pbar.update(1)
    ...
    if cov.dtype != object:
        ...
        # and setting one half of the covariance to the identity.
        ...
        block_diag[outsize:, outsize:] = np.eye(outsize, dtype=block_diag.dtype, device=device)[...,None]
        block_diag[:outsize, outsize:] = 0
        block_diag[outsize:, :outsize] = 0

        return entropy_from_covariance(block_cov, epsilon=0)
