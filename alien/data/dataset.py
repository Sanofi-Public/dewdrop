...
from ..utils import add_slice, is_0d, reshape, update_copy, Identity
...
def find_dataset_class(data, *args, convert_sequences=True, **kwargs):
    ...
    if isinstance(data, TeachableDataset):
        ...
        if data.dtype == object and not isinstance(data[0], np.ndarray):
            return ObjectDataset
        else:
            return NumpyDataset
    elif isinstance(data, tuple):
        return TupleDataset.__new__
...
class ShuffledDataset(TeachableWrapperDataset):
    ...
    def y(self):
        try:
            ...
            raise NotImplementedError(
                f"Dataset of type {type(self.data)} does not have a y attribute."
            ) from err
...
def compute_bdim(old_shape, old_bdim, new_shape):
    ...
    for bdim, d in enumerate(new_shape):
        ...
        if size == b_size:
            ...
            raise ValueError(
                "New shape must have initial axes with total size equal to the original batch size."
            )
...
class NumpyDataset(ArrayDataset):
    ...
    def __init__(self, data, **kwargs):
        if isinstance(data, np.ndarray) and data.dtype == object and isinstance(data[0], np.ndarray):
...
class ObjectDataset(NumpyDataset):
    ...
    def shape(self):
        try:
            ...
            feat_shape = (None,)
        return *self.data.shape, *feat_shape

    def reshape(self, *shape, index=None, **kwargs):
        """Think I can make this work..."""
        raise NotImplementedError(
            "Reshape is not supported by ObjectDataset due to variable-length inputs."
        )
...
class DictDataset(TeachableWrapperDataset):
    ...
    def __init__(self, data={}, convert_sequences=True, bdim=1, has_Xy=None, **kw_data):  # NOSONAR
        ...
        super().__init__(
            None, bdim=bdim, has_Xy=bool({"X", "x", "y"} & set(data)) if has_Xy is None else has_Xy
        )
        self.data = {
    ...
    def dtype(self):
        ...
        if not all([d == dts[0] for d in dts]):
            raise TypeError(f"To call self.dtype, all subdtypes must be equal.\n" \
                f"In this case, the subdtypes are:\n{{k: v.dtype for k, v in self.data.items()}}")
        return dts[0]
