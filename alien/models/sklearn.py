...
if VERSION < (1, 3):
    ...
    def _fit_stage(
        ...
        raw_predictions *= 1 - learning_rate * self.loss.regularization
...
class LangevinMixin:
    ...
    def __init__(self, n_classes=None, temperature=0.5, regularization=0.5, random_seed=None):
    ...
    def gradient(
        ...
        gradient_out += self.rng.normal(scale=self.temperature, size=ng_res.shape)
        return gradient_out
...
class SKLearnModel(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sklearn_kwargs = {}
        if "ensemble_size" in kwargs.keys():
...
class SKLearnEnsembleModel(SKLearnModel, EnsembleModel):
    ...
    You can pass in all the usual arguments to an :class:`EnsembleRegressor` or :class:`EnsembleClassifier`,
    ...
    def predict_samples(self, X, n: int = None, *args, **kwargs):
        ...
        res_tensor = res_tensor.unsqueeze(-1) if res_tensor.dim() < 3 else res_tensor
    ...
    def _forward(self, X, *args, **kwargs):
        """Raises NameError."""
        raise NameError("This method is not implemented for RandomForestRegressor.")
...
class SKLearnEnsembleClassifier(SKLearnEnsembleModel, Classifier):
    ...
    def predict_samples(self, X, n: int = 1, *args, **kwargs):
        ...
        res_tensor = torch.stack(preds, 1)
        # add last dimension if tensor has < 3 dims (single elements in last dim)
        res_tensor = res_tensor.unsqueeze(-1) if res_tensor.dim() < 3 else res_tensor
    ...
    def call_predict_fn(self, model, X):
        predict_fn = getattr(model, self.get_predict_fn())  # get the specific function to call
        return torch.tensor(predict_fn(X))
...
class RandomForestRegressor(SKLearnEnsembleModel):
    ...
    Args:
        ...
        max_depth, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_features, max_leaf_nodes, min_impurity_decrease, bootstrap, oob_score, n_jobs, random_state, verbose, warm_start, ccp_alpha, max_samples, monotonic_cst: See the `SKLearn docs <link URL>`_
    """
    ...
    def build_models(self):
        ...
        for i in range(self.ensemble_size):
            ...
            if self.loss in [LangevinMultinomial]:
                m._loss = self.loss(self.n_classes)
            else:
                m._loss = self.loss()
