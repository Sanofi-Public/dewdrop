...
from ..utils import concatenate, dot_last, chunks
from .selector import SampleSelector
from ..models import Output, Classifier, Regressor
    ...
    def _select(
        ...
        **kwargs
        ...
        with tp.no_grad():
            ...
            V_l = self.grad_embedding(labelled_samples, output_type=getattr(self.model, 'wrapped_output', None)).reshape((len(labelled_samples), -1, V_u.shape[-1]))
            ...
            for _ in range(min(int(batch_size * self.oversample), len(V_u))):
                ...
                score = tp.trace(
                    V_u.swapaxes(1, 2) @ M_inv @ F @ M_inv @ V_u @ A_inv,
                    axis1=-2, axis2=-1)
            ...
            for _ in range(len(ind_s) - batch_size):
                ...
                if A.shape[-1] == 1:
                    ...
                    A_inv = tp.linalg.pinv(A)
                score = tp.trace(
                    V_s.swapaxes(1, 2) @ M_inv @ F @ M_inv @ V_s @ A_inv,
                    axis1=-2, axis2=-1)
...
def fisher_cross_entropy(model, X, output_type='logit', batched=False, batch_size=None, batch_total_size=None, return_embedding=True, normalize=False):
    ...
    F = fisher_from_grad_embeddings(V, batched=batched, batch_size=batch_size, batch_total_size=batch_total_size, normalize=normalize)
...
def exp_grad_embedding(model, X, output_type='logit'):
    ...
    p = tp.softmax(out, axis=-1) if Output(output_type) == Output.LOGIT else out
