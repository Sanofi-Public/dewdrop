...
def expected_improvement(
    samples, model, labelled_samples, margin=0.03, multiple=1, return_predictions=False
):
...
class ExpectedImprovementSelector(SampleSelector):
    ...
    def _select(self, samples=None, labelled_samples=NEED, batch_size=None):
        y = labelled_samples.y
        benchmark = np.max(y) + self.margin * np.std(y)
