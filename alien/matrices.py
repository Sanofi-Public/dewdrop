...
from .utils import new_shape, expand_ellipsis
...
class LazyMatrix:
    ...
    def __init__(self, shape, lazy_dim=0, buffer_size=None, use_buffer=True, buffer=None, device=None, preindex=None, **kwargs):
        self.shape = shape
        self.ndim = len(shape)
        self.lazy_dim = lazy_dim
        self.use_buffer = use_buffer
        self.preindex = tp.asarray(preindex) if preindex is not None else None
        if use_buffer and buffer_size != False:
            if buffer is None:
    ...
    def __getitem__(self, indices):
        ...
        if self.lazy_dim > len(indices) or (isinstance((u2m_indices := indices[self.lazy_dim]), slice) and u2m_indices == None_slice):
            r = copy(self)
        ...
        if isinstance(u2m_indices, slice):
            ...
            stop = self.shape[self.lazy_dim] if u2m_indices.stop is None else min(u2m_indices.stop, self.shape[self.lazy_dim])
        ...
        if len(new_m_indices):
            if self.preindex is None:
                ...
                new_buffer_view[:n_new] = tp.moveaxis(self.compute(new_m_indices), self.lazy_dim, 0)[(None_slice, *self.preindex)]
        ...
        old_tail_indices = tp.argwhere((new_m2b_indices < 0) & (self.m2b_indices >= 0))[:self.buffer_size-n_new-n_old, 0]
        ...
        buffer_indices = indices[:self.lazy_dim] + (self.m2b_indices[
                u2m_indices[0] if is_single_index else u2m_indices
            ],) + indices[self.lazy_dim+1:]
...
class EnsembleMatrix(LazyMatrix):
    ...
    def __init__(self, ensemble, prior=None, block_size=None, **kwargs):
